...
def move():
    ...
    velocity_pub = rospy.Publisher('/cmd_vel_nav', geo_msg.Twist, queue_size=1)
    speed = 0.12
    rospy.sleep(1)
    while True:
        if __isRunning:
            ...
            if haved_detect:
                ...
                haved_detect = False
                if tag_id == '1':
#                     shared_data[1].linear.x = 0
#                     shared_data[1].angular.z = 0
#                     velocity_pub.publish(shared_data[1])
#                     time.sleep(6)
                    os.system("mpg123 " + "arrive_A.mp3")
...
#                     time.sleep(4)
                    print("detect tag 3")
                    if target_point == "A":
                        shared_data[1].linear.x = speed
                        shared_data[1].angular.z = 0
                        velocity_pub.publish(shared_data[1])
                        time.sleep(2)
                        shared_data[1].linear.x = speed / 6
                        w = speed*4
                        max_angle = math.radians(90)
                        shared_data[1].angular.z = -w
                        velocity_pub.publish(shared_data[1])
                        time.sleep(3.5)
                    if target_point == "B":
                        shared_data[1].linear.x = speed
...
#                 ,height=PuppyPose['height'], roll=PuppyPose['roll'], pitch=PuppyPose['pitch'], yaw=PuppyPose['yaw'], run_time = 500)
                ...
                velocity_pub.publish(shared_data[1])
                print("turn")
#                     time.sleep(max_angle / w )
                print("start turn")
#                 self.timestamp = time.time() + (max_angle / w )
                time.sleep(3.5)
...
#             
            ...
            if line_centerx != -1:
                if abs(line_centerx - img_centerx) <= 50:
                    ...
                    PuppyMove['x'] = 11
                    PuppyMove['yaw_rate'] = math.radians(-12) # was 15 before
...
if __name__ == "__main__":
    ...
    #rospy.init_node(node, log_level=rospy.INFO)
    rospy.on_shutdown(cleanup)
    
    PP = rospy.get_param('/puppy_control/PuppyPose')
#    PuppyPose = PP['LookDown_10deg'].copy()
    PuppyPose = PP['LookDown_10deg'].copy()
#    PuppyPose = {'roll':math.radians(0), 'pitch':math.radians(-10), 'yaw':0.000,'height':-10, 'x_shift':-0.6, 'stance_x':0, 'stance_y':0}
